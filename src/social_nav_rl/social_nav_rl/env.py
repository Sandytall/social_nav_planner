...
class MockBackend:
    ...
    def __init__(self, ep: EpisodeConfig, dt: float):
        self.ep = ep
        self.dt = dt
        self._people = []
        self._t = 0.0
    ...
    def obstacle_hit(self, x: float, y: float, radius: float) -> bool:
        for (xmin, ymin, xmax, ymax) in self._obstacles:
...
class SocialNavEnv(gym.Env if _GYM else object):
    ...
    def __init__(self, ep: EpisodeConfig = None, obs_cfg: ObsConfig = None,
        ...
        self.backend = backend or MockBackend(self.ep, self.limits.dt)
        self._steps = 0
    ...
    def _obs(self, humans, preds=None):
        if preds is None:
            preds = self.backend.predictions(humans)
        obs, mask = self.adapter.build(self._robot_state(), humans, preds)
