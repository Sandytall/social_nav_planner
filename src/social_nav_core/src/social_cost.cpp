...
{
  // Transform the query point into the human's local frame (§10): rotate the world
  ...
  // sigma_x depends on whether the point is in front of or behind the human (§9).
  const double sigma_x = longitudinal >= 0.0 ? params.front_sigma : params.rear_sigma;
  const double sigma_y = params.side_sigma;

  // Guard against non-positive sigmas so a misconfiguration cannot divide by zero (§54).
